...
export const Route = createFileRoute("/.lovable/oauth/consent")({
  ...
  head: () => ({ meta: [{ title: "Authorize app — Ajiko Fidelis" }, { name: "robots", content: "noindex" }] }),
});
    ...
    <main className="flex min-h-screen items-center justify-center bg-background px-4 py-16">
      <div className="w-full max-w-md rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold tracking-tight text-foreground">
          Connect {clientName} to Ajiko Fidelis
        </h1>
        {email && (
          <p className="mt-1 text-xs text-muted-foreground">Signed in as {email}</p>
        )}
        <p className="mt-4 text-sm text-muted-foreground">
          This lets {clientName} call this site&apos;s agent tools as you.
        </p>
